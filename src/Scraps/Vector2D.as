package Base {
	...
	public class Vector2D
	{
		...
		/** Calculates the component of a vector projected onto this vector, 
		 * creating a new Vector2D instance.**/
		
		public function componentOnto(v2:Vector2D) : Vector2D 
		{
			var dot : Number = this.dotProd(v2)/v2.lengthSQ;
			return v2.multiply(dot);
		}
		
		/** Creates a new Vector2D instance, with endpoint of type Point**/
		
		public static function vectorToPoint(p : Point) : Vector2D 
		{
			return new Vector2D(p.x , p.y );
		}
		
		/** Calculates the vector connecting two points of type Point from point1 to point2, 
		 * creating a new Vector2D instance.**/
		
		public static function pointConnection(p1 : Point,p2 : Point) : Vector2D 
		{
			return new Vector2D(p2.x - p1.x, p2.y - p1.y);
		}
		
		/** Calculates the vector connecting the endpoint op this vector to the endpoint
		 * of another vector
		 * creating a new Vector2D instance.**/
		
		public function connection(v : Vector2D) : Vector2D 
		{
